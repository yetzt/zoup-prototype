...
const vidsize = require("./video-size-remote.js");
...
link.prototype.fetch = function(u, fn){
	...
	// tumblr freaks out on head requests with compression
	const compressed = !/tumblr\.com$/.test(self.url.hostname)
	
	needle.head(u, { ...needle_opts, compressed: compressed }, function(err, resp, data){
		...
		switch (mime_type) {
			...
			case "application/xhtml+xml":
				switch (self.url.hostname.replace(/^www\./,'')) {
					...
					default:
						switch (self.url.hostname.replace(/^[^\.]+\./,'')) {
							case "tumblr.com":
								self.tumblr(u, fn);
							break;
							default:
			...
			case "video/webm":
				...
				vidsize(u, function(err, video){

					if (!err) return fn(err, { type: "link", url: u });
...
link.prototype.twitter = function(u, fn){
	...
	if (/^https:\/\/(mobile\.)?twitter\.com\/([a-zA-Z0-9\_]+)\/status\/([0-9]+)([\?\/].*)?$/.test(u)) { // embed whole tweet instead of specific media FIXME

		self.get("https://cdn.syndication.twimg.com/tweet?id="+(RegExp.$3), function(err, resp, data){ // hopefully this api stays usable
...
link.prototype.youtube = function(u, fn){
	...
	self.get("https://www.youtube.com/oembed?format=json&url="+encodeURIComponent(u), function(err, resp, data){
		...
		fn(null, {
			type: "link.embed",
			url: u,
			src: data.html.replace(/^.*<iframe [^>]*src="([^"]+)"[^>]*><\/iframe>.*$/,'$1').replace(/youtube\.com/,'youtube-nocookie.com')+'&amp;modestbranding=1&amp;rel=0&amp;showinfo=0&amp;iv_load_policy=0',
			title: data.title,
			width: 1000,
			height: (data.height/data.width)*1000, // scale accordingly
		});
...
link.prototype.tumblr = function(u, fn){
	const self = this;
	
	self.get("https://www.tumblr.com/oembed/1.0?url="+encodeURIComponent(u), function(err, resp, data){
		if (err || Math.floor(resp.statusCode/100) !== 2) return fn(err || new Error("HTTP Status "+resp.statusCode), null);
		try {
			data = JSON.parse(data);
		} catch (err) {
		...
		return fn(null, {
			type: "link.embed",
			url: u,
			src: data.html.replace(/^.*<div [^>]*data-href="([^"]+)"[^>]*>.*$/,'$1'),
			width: data.width,
			height: 500, // FIXME ¯\_(ツ)_/¯
		});
