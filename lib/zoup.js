...
const zoup = function(opts){ // opts is the config object
	...
	self.feeds.ensureIndex({ fieldName: 'type' });
	self.feeds.ensureIndex({ fieldName: 'checked' });
	self.feeds.ensureIndex({ fieldName: 'feed_url' });
	self.feeds.ensureIndex({ fieldName: 'last_update' });
...
// create post
zoup.prototype.publish = function(post, fn){
	const self = this;
	...
	self.ingest(post, function(err, post){
		...
		self.posts.insert(post, function(err, post){
			...
			self.emit("publish", self.item(post));
...
// update post
zoup.prototype.republish = function(post, fn){
	const self = this;
...
// delete post
zoup.prototype.unpublish = function(id, fn){
	const self = this;
	
	self.posts.remove({ _id: id }, {}, function(err){
		if (err) return fn(err);
		self.emit("unpublish", { id: id });
		return fn(null, id);
	});
...
// follow remote feed
zoup.prototype.follow = function(u, fn){
	const self = this;
	
	// check for double feeds
	self.feeds.find({ feed_url: u }).count(function(err, c){
		if (err) return fn(err);
		if (c > 0) return fn(new Error("Already subscribed"));

		// get feed
		feed.check(u, function(err, data){
			if (err) return fn(err);
			...
			self.feeds.insert({
				...
				if (err) return fn(err);
				self.emit("follow", data.feed_url);
				return fn(null, data);
			});
...
zoup.prototype.unfollow = function(u, fn){
	const self = this;
	debug("unfollow %s", u);
	self.feeds.remove({ feed_url: u }, {}, function (err, c) {
		if (err) return fn(err);
		if (c === 0) return fn(new Error("No subscription"));
		self.emit("unfollow", u);
		return fn(null, { feed: u });
	});
...
// list subscriptions
zoup.prototype.subscriptions = function(fn){
	const self = this;
	self.feeds.find({}).sort({ subscribed: -1 }).exec(function(err, subscriptions){
		if (err) return fn(err);
		return fn(null, subscriptions);
	});
	return this;
};
